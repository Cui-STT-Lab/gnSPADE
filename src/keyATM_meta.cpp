...
void keyATMmeta::iteration()
{
  ...
  for (int it = iter_start; it < iter; ++it) {
    ...
    if (r_index % thinning == 0 || r_index == 1 || r_index == iter) {
      //Rcpp::Rcout << "sampling store is implemented." << std::endl;
    ...
    if ((r_index % 100) == 0) {
      Rcpp::Rcout << "Iteration: " << r_index << " completed." << std::endl;
    }
...
{
  //Rcpp::Rcout << "parameters_store is implemented." << std::endl;
  if (store_theta) {
    //Rcpp::Rcout << "store_theta is implemented." << std::endl;
...
void keyATMmeta::store_phi_iter(int r_index)
{
  W_tables = stored_values["W_tables"];
  NumericMatrix W_table = Rcpp::wrap(n_kv_noWeight);
  //NumericMatrix W_table = Rcpp::wrap(n_dk_noWeight);
  W_tables.push_back(W_table);
  stored_values["W_tables"] = W_tables;
}
...
//void keyATMmeta::generateSentNeighArr(int num_doc, List word_neighbors)
//{}
