...
class keyATMmeta
{
  ...
  public:
    ...
    void store_phi_iter(int r_index);
    void store_pi_iter(int r_index);

    virtual void verbose_special(int r_index);

    virtual double loglik_total() = 0;
    //virtual VectorXd computeNeiProb(int doc_id, int word_name, int num_topics) = 0;
    void generateSentNeighArr(int num_doc, List word_neighbors) {
        // Placeholder implementation to avoid runtime issues
        Rcpp::Rcout << "generateSentNeighArr called with " << num_doc << " documents." << std::endl;
        ...
        Rcpp::Rcout << "Finish generateSentNeighArr " << num_doc << " documents!!!" << std::endl;
    };
    //virtual void generateSentNeighArr(int num_doc, List word_neighbors) = 0;
