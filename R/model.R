...
keyATM_initialize <- function(docs, model, no_keyword_topics,
                       ...
                       priors = list(), options = list(), Z = NULL)
{
  ##
  ## Check
  ##
  ...
  if (options$store_phi) {
    stored_values$W_tables <- list()
  }
...
check_arg_priors <- function(obj, model, info)
{
  check_arg_type(obj, "list")
  # Base arguments
  allowed_arguments <- c("beta", "eta_1", "eta_2", "eta_1_regular", "eta_2_regular", "phi", "lambda")
...
check_arg_options <- function(obj, model, info)
{
  ...
  allowed_arguments <- c("seed", "llk_per", "thinning",
                         ...
                         "prune", "store_theta", "store_phi", "slice_shape",
  ...
  # Store phi
  if (is.null(obj$store_phi)) {
    obj$store_phi <- 0L
  } else {
    obj$store_phi <- as.integer(obj$store_phi)
    if (!obj$store_phi %in% c(0, 1)) {
      cli::cli_abort("An invalid value in `options$store_phi`")
    }
  }
